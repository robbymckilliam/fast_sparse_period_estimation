...
for( N <- List(30,1200) ) {
  ...
  for( m <- List( 1, 10 ) ) {
    ...
    for( q <- List(1.5, 4.0) ) runsim(varbs, N, () => discretedist, () => new QuantisedPeriodogramFFT(N,T_min,T_max,q*f_max), "QuantisedPeriodogramFFTN" + N + "q" + q + "geom" + m)
...
def runsim(varbs : Seq[Double], N : Int, discretedist : () => DiscreteRandomVariable, estf : () => PRIEstimator, name : String) {

  val vars = varbs.map(db => scala.math.pow(10.0, db/10.0)) //list of variances
  val signalgenerators = vars.map{ v => new SparseNoisyPeriodicSignal(N, T_0, theta_0, discretedist(), noisedist(v)) }    
...
def runMonteCarlo(est : PRIEstimator, gen : SparseNoisyPeriodicSignal, iterations : Int) : Double = {
  val T_0 = gen.getPeriod //get the true period
  if( est.getLength != gen.getLength ) throw new RuntimeException("Lengths of estimator and generator don't match")
